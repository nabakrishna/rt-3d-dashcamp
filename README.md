...
```bash
git clone https://github.com/you/dashcam-perception.git
cd dashcam-perception
```
...
| Symptom | Fix |
|---------|-----|
| `ImportError: numpy` on Python 3.12 | Upgrade: `pip install "numpy>=1.26.4"` |
| `Cannot open video` | Check path; try re-encoding with `ffmpeg -i in.mp4 out.mp4` |
| CUDA out-of-memory | Switch to `--device cpu` or reduce `DEPTH_INPUT_W/H` |
| Slow CPU inference | Normal — depth model is the bottleneck; expect 2–8 fps on CPU |
| `ModuleNotFoundError: supervision` | `pip install supervision>=0.21.0` |
| Black BEV panel | Depth model may not have downloaded yet — check internet access |
...
Model weights are subject to their respective licences (YOLOv8: AGPL-3.0,
